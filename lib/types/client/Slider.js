...
export function Slider({ label, value, min, max, display, onChange }) {
    return (_jsxs("label", { className: css.row, children: [_jsx("span", { className: css.label, children: label }), _jsx("input", { className: css.range, type: "range", min: min, max: max, value: value, "aria-label": label, onChange: (event) => { onChange(Number(event.currentTarget.value)); } }), _jsx("span", { className: css.value, children: display })] }));
}
