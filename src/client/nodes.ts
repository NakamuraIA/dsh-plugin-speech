 ...
 */
function assistantSource(event: SessionEvent<'assistant/message'>): string {
 ...
 */
export function registerSpeechText(ctx: ClientContext, controller: SpeechController): void {
  const definition: ConversationNodeDefinition<{ readonly messageId: string }> = {
    kind: KIND,
    match(event) {
